...
make_plot.popdens <- function(preds = NULL, species = NULL, add_se = FALSE, 
  ...
  tiff(paste0("./plots/",species, 
              ifelse(intercept, "_intercept", "_slope"),".tiff"), 
       height = 4, width = 4, units = "in",
       res = 400, compression = "lzw")
  par(mar = c(3.5,4,0.5,0.5))
  if(intercept){
  plot(1~1, type ="n", bty = 'l', xlab = "", ylab = "", xaxt = "n",
       yaxt = "n", ylim = c(0,1), xlim = c(100,1300))
  }else{
    plot(1~1, type ="n", bty = 'l', xlab = "", ylab = "", xaxt = "n",
         yaxt = "n", ylim = c(-2,2), xlim = c(100,1300))
  }
  x1 <- x_range
  x2 <- rev(x1)
  y1 <- my_pred[1,]
  ...
  axis(1, at = seq(100, 1300, 200), labels = F, tck = -0.025)
  axis(1, at = seq(100, 1300, 100), labels = F, tck = -0.025/2)
  mtext(text = seq(100, 1300, 200), 1, line = 0.35, at = seq(100,1300,200))
  ...
  mtext(text = "Average occupancy rate", 2, at = 0.5, cex = 1.2, line = 2.6)
    ...
    mtext(text = "Urbanization slope", 2, at = 0, cex = 1.2, line = 2.6)
  }
  mtext(text = "Population density in study area", 1, at = 700, cex = 1.2, line = 1.7)
...
make_plot.habitat <- function(preds = NULL, species = NULL, add_se = FALSE,
  ...
  my_se <- preds$cse
  # windows(4,4)
  tiff(paste0("./plots/",species,ifelse(intercept, "_intercept", "_slope"),
